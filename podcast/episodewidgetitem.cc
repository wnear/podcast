    ...
    do {
        if (m_data->getState() == pd::MediaFileDownloading) {
            if (total == 0) {  // filesize not have meaning.
                ...
                if (total)  // filesize not have meaning ,but net_total has, still can
                            // save.
                    total += cur;
            }
...
EpisodeWidgetItem::EpisodeWidgetItem(EpisodeData *data, QWidget *parent)
    ...
    auto description = fm.elidedText(m_data->description, Qt::ElideRight,
                                     fm.horizontalAdvance("a") * 200);
